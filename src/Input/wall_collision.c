...
/*   Updated: 2025/06/20 13:30:10 by ksinn            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	is_wall_collision(t_game *game, double x, double y)
{
	...
	// Calculate the range of map cells that could potentially collide
	...
	for (map_y = min_y; map_y <= max_y; map_y++)
	{
		...
		{
			// Check if this map position is valid and is a wall
			if (!is_valid_position(game, map_x, map_y))
				return (1); // Treat out-of-bounds as collision
			if (game->map->map[map_y][map_x] == '1')
			{
				// Calculate closest point on the wall cell to the player
				closest_x = fmax(map_x, fmin(x, map_x + 1.0));
				closest_y = fmax(map_y, fmin(y, map_y + 1.0));
				// Calculate distance from player center to closest point on wall
